...
if __name__ == "__main__":
    ...
    for url in urls:
        ...
        #action_process = Process(target=parser_file(file,mode))
        #action_process.start()
        #matched=action_process.join(timeout=120)
                ...
                # find same thing in multiple context
                    ...
                    # remove duplicate
                        ...
                        # --
                    			...
                    			body += '</a><div class="container">%s</div></div>'%(match.get('context')[0] if match.get('context') != [] else  '')
                    			body = body.replace(match.get('context')[0] if match.get('context') != [] else '','<span style="background-color:yellow">%s</span>'%(match.get('context')[0] if match.get('context') != [] else ''))
                		output += header + body 
        except:
        	continue
    if args.output != 'cli':
